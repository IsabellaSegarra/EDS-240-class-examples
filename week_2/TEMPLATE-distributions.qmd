...
#..........................import data...........................
mko <- read_csv(here::here("week_2", "data", "mohawk_mooring_mko_20250117.csv"))
...
mko_clean |> 
  ...
  ggplot(aes(x = Temp_bot, fill = month_name)) +
  geom_histogram(position = "identity", alpha = 0.5)+ 
  facet_wrap(~month_name) # Facet by month 
...
Overlay histogram and density plots to check that smoothing assumptions of the density curve align with the actual data distribution. In a histogram, the bins are the size of the data distribution whereas density curve is the concentration of the data under a curve with an area of 1. 
:::
...
:::
This is helpful if you are interested in reversing the order only for this graph. If you do it earlier in the data wrangling process, then that limits your analysis to this order when you are plotting later. 
...
```{r}
# Data explore
unique(palmtrees$fruit_shape)

# Bar chart of fruit shape per palm tree 
palmtrees %>%
  group_by(palm_subfamily, fruit_shape) %>%
  summarise(count = n()) %>%
  drop_na() %>% 
  ggplot(aes(x = palm_subfamily, y = count, fill = fruit_shape)) +
  geom_col() +
  theme_bw()
