...
#..........................import data...........................
iwa_data <- read_csv(here::here("week_3", "data", "combined_iwa-assessment-outputs-conus-2025_CONUS_200910-202009_long.csv"))
...
subregion_names <- tribble( #tidyverse dataframe = tibble 
...
ca_region <- iwa_data |> 
  ...
  # Similar to stringr; experimental package
...
ca_region %>% 
  ...
  mutate(subregion_name = fct_reorder(.f = subregion_name, .x = mean_sui_2010_2020)) %>% 
  ggplot(aes(x = mean_sui_2010_2020, y = subregion_name)) +
    # add geompoint for top of lollipops
  geom_point() +
    # draw lines with geom_linerange, requires aes() to "map" where to draw the lines
...
ggplot(ca_region, aes(x = subregion_name)) +
  geom_bar() +
  coord_flip() + # does not reverse mappings, thats why you use scale_Y_continuous 
...
sui_severity <- ca_region |>
  ...
  # month.abb = assign abbreviations from month numbers 
  mutate(month = month.abb[month],
         # refactor the month levels according to water years 
  ...
  mutate(sui_category = factor(factor(sui_category, 
...
ggplot(sui_severity, aes(x = month, y = tot_num_sui, fill = sui_category)) +
  geom_col(position = "fill") + # default = idenity, fill = 0 to 1  
...
heatmap_order <- heatmap_data |> 
  ...
  # reorder subregion by the values in the order column 
...
ggplot(heatmap_order, aes(x = year, y = subregion_name, fill = annual_mean_sui)) +
  ...
  theme(
    ...
    axis.title = element_blank() # remove panel grid
  )
...
ggplot(sbc_subbasin_monthly) +
  # draw lines between points based from y (month) to mean_avail
  geom_linerange(aes(y = month, xmin= mean_consum, xmax= mean_avail)) +
